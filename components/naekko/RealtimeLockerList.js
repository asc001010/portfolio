...
export default function RealtimeLockerList({ isOpen, onClose }) {
  ...
  useEffect(() => {
    ...
    endValue.setDate(endValue.getDate() + 30);
    const end = endValue.toISOString().split('T')[0];
  ...
  const { finalAmount, diffDays, discountedAmount, totalAmount } = useMemo(() => {
    if (!startDate || !endDate || !selectedLocker) {
      return { finalAmount: 0, diffDays: 0, discountedAmount: 0, totalAmount: 0 };
    }
    ...
    const dailyRate = selectedLocker.price / 30;
    const total = Math.floor(dailyRate * days);
    
    let discounted = total;
    if (days >= 360) discounted = total * 0.8;
    else if (days >= 180) discounted = total * 0.9;
    else if (days >= 90) discounted = total * 0.95;

    const final = useSubscription ? 0 : Math.floor(discounted);
  ...
  }, [startDate, endDate, selectedLocker, useSubscription]);

  if (!mounted) return null;

  const currentLockers = selectedBranch ? (LOCKERS_DATA[selectedBranch.id] || []) : [];
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[120] flex items-end sm:items-center justify-center p-0 sm:p-4 overflow-hidden">
          ...
          <motion.div
            ...
            className="relative bg-white w-full max-w-4xl sm:rounded-[2.5rem] rounded-t-[2.5rem] shadow-2xl overflow-hidden h-[92vh] sm:h-auto sm:min-h-[650px] sm:max-h-[90vh] flex flex-col will-change-transform"
          ...
          >
            ...
            <div className="px-6 sm:px-8 py-5 sm:py-6 border-b border-zinc-100 flex items-center justify-between bg-white shrink-0">
              <div className="flex items-center gap-3 sm:gap-4">
                {step > 0 && (
                  <button 
                    ...
                    className="p-1.5 sm:p-2 hover:bg-zinc-50 rounded-xl transition-colors text-zinc-400 active:scale-95"
                  >
              ...
              <button onClick={onClose} className="p-1.5 sm:p-2 hover:bg-zinc-50 rounded-full transition-colors text-zinc-300 active:rotate-90 active:scale-90 duration-200">
                  ...
                  >
                    {BRANCHES.map((branch) => (
                      <div 
                        key={branch.id}
                        role="button"
                        tabIndex={branch.lockersCount > 0 ? 0 : -1}
                        onClick={() => branch.lockersCount > 0 && (setSelectedBranch(branch), setStep(1))}
                        onKeyDown={(e) => {
                          if (e.key === 'Enter' || e.key === ' ') {
                            branch.lockersCount > 0 && (setSelectedBranch(branch), setStep(1));
                          }
                        }}
                        className={`group p-6 rounded-[2rem] border transition-all cursor-pointer flex flex-col justify-between min-h-[160px] active:scale-[0.98] ${
                ...
                {step === 1 && (
                  ...
                  >
                    ...
                    <div className="w-full lg:w-72 mt-2 lg:mt-0">
                      <div className="bg-[#1B2435] p-6 rounded-[2rem] lg:rounded-3xl text-white shadow-lg lg:sticky lg:top-0">
                        ...
                        <button 
                          ...
                          onClick={() => setStep(2)}
                          className="w-full py-4 bg-blue-500 text-white rounded-2xl font-black text-sm hover:bg-blue-600 transition-all shadow-lg shadow-blue-500/20 disabled:opacity-30 disabled:cursor-not-allowed flex items-center justify-center gap-2 active:scale-95"
                        >
                          날짜 선택하기
                  ...
                  >
                    ...
                    <div className="bg-white p-8 rounded-[2.5rem] border border-zinc-100 shadow-sm">
                      ...
                      <div className="space-y-4 mb-8">
                        ...
                        {!useSubscription && totalAmount > discountedAmount && (
                      ...
                      <button className="w-full py-5 bg-[#1B2435] text-white rounded-[1.5rem] font-bold text-base hover:bg-black transition-all flex items-center justify-center gap-3 active:scale-[0.98]">
...
function LockerCard({ locker, isSelected, onSelect }) {
  ...
  return (
    <div 
      onClick={onSelect}
      className={`p-4 sm:p-6 rounded-[1.8rem] sm:rounded-[2rem] border transition-all flex items-center gap-4 sm:gap-6 cursor-pointer active:scale-[0.98] ${
