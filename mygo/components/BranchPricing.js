    ...
    <section id="branch" className="py-24 bg-surface-low">
      <div className="container mx-auto px-6 max-w-6xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 items-center">
          {activeBranch.lockers.map((locker, lIdx) => (
            <div 
              key={locker.id}
            ...
            >
              ...
              <ul className="space-y-3 mb-8">
                {locker.features.map((feature, fIdx) => (
                  <li key={`${locker.id}-feat-${fIdx}`} className="flex items-center gap-2 text-xs font-bold">
      ...
      <BranchDetailModal 
        key={`branch-detail-modal-${activeBranch.id}`}
        isOpen={isModalOpen} 
